...
function initializeServiceWorker() {
  ...
  if('serviceWorker' in navigator) {
    ...
    window.addEventListener('load', async function() {
      ...
      // STE PS B6 ONWARDS WILL BE IN /sw.js
    });
  }
  ...
  const myPromise = new Promise((resolve, reject) => {
    ...
    RECIPE_URLS.forEach(async (recipeUrl) => {
      ...
      try {
        ...
        //            "await" again 
        let fetchedRecipe = await response.json();
        // A8. TODO - Add the new recipe to the recipes array
        fetchedRecipesArray.push(fetchedRecipe)
        ...
        //            resolve() method.
        if(fetchedRecipesArray.length == RECIPE_URLS.length) {
          saveRecipesToStorage(fetchedRecipesArray);
          resolve(fetchedRecipesArray);
        }        
      } catch (error) {
